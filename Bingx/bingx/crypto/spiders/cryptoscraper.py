...
class CryptoscraperSpider(scrapy.Spider):
    ...
    async def parse(self, response,p):
        ...
        async with async_playwright() as pw:
            ...
            xpath = '//div[@class="mtb-base-p-item" and text()="Trades"]'
            btn="Trades"
            pp= page.locator(f'//ul[@class="multi-tab-bar mtb-small"]//li[@class="mtb-base-p-item-wrap"]').filter(has_text="Trades")
            await pp.click()
            await page.wait_for_timeout(10000)
            result_list=[]
            old_result=[]
            cnt=0
            while True:
                xpath = '//div[@class="history-list-wrap hide-scroll-bar"]//div[@role="listitem"]//span'
                elements_amount = await page.query_selector_all(xpath)
                #print(elements_amount)
                result_lists=[]
                for element in elements_amount:
                    ...
                    if class_name == "price buy":
                        #print(text_contentt)
                        result_lists.append("buy")
                    if class_name == "price sell":
                        #print(text_contentt)
